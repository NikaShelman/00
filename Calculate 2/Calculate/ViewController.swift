...
class ViewController: UIViewController {
    ...
    private func performOperation() {
        ...
        if let text = display.text, let value = Double(text.replacingOccurrences(of: ",", with: ".")) {
            ...
            if let op = operation {
                ...
                switch op {
                case "+":
                    if let lastOperation = operation, lastOperation == "%" {
                                        let percentageValue = value / 100.0 * result // вычисляем процент
        ...
        // форматирую результат так, чтобы целочисленные значения высвечивались как Int, а дробные разделялись символом "," вместо точки и ограничивали значение 15-ю знаками после запятой. Так же говорю swift о том, что большие числа, имеющие разряды тысяч, миллионов и т. д. разделялись по разрядам пробелом, а не запятой, чтобы это не ломало вычислительный процесс
    ...
    @IBAction func operationButtonClick(_ sender: UIButton) {
        ...
        // создаю условие, при котором введенное значение будет автоматически переведено в 1% при нажатиии на знак %
        if title == "%" {
            if let text = display.text, let value = Double(text.replacingOccurrences(of: ",", with: ".")) {
